...
export const DetailBerita: React.FC<DetailBeritaProps> = ({ berita, onBack }) => {
  ...
  useEffect(() => {
    ...
    // Perbarui tag Open Graph dan Twitter Card secara dinamis agar menampilkan thumbnail website di sosmed
    const originalTitle = document.title;
    document.title = `${berita.judul} - Desa Nyurlembang`;
    ...
    setMeta('og:title', berita.judul);
    setMeta('og:description', berita.ringkasan || berita.judul);
    setMeta('og:image', berita.gambar_url);
    setMeta('og:url', window.location.href);
    setMeta('og:type', 'article');
    setMeta('twitter:card', 'summary_large_image');
    setMeta('twitter:title', berita.judul);
    setMeta('twitter:description', berita.ringkasan || berita.judul);
    setMeta('twitter:image', berita.gambar_url);
  ...
  const handleShareSosmed = (platform: 'wa' | 'fb' | 'x' | 'telegram' | 'linkedin' | 'pinterest') => {
    ...
    const text = `${berita.judul} - Kabar Desa Nyurlembang`;
    const image = berita.gambar_url;
    ...
    switch (platform) {
      ...
      case 'linkedin':
        ...
        break;
      case 'pinterest':
        window.open(`https://pinterest.com/pin/create/button/?url=${encodeURIComponent(currentUrl)}&media=${encodeURIComponent(image)}&description=${encodeURIComponent(text)}`, '_blank');
        break;
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-300">
      {/* Tombol Kembali & Menu Berbagi ke Semua Sosmed (Tanpa Salin Link) */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <button
          onClick={onBack}
          className="inline-flex items-center gap-2 px-4 py-2 bg-white hover:bg-slate-100 text-[#0D2A4A] rounded-xl text-xs font-bold border border-slate-200 transition-colors shadow-xs self-start"
        >
      ...
      {/* Artikel Utama */}
      <article className="card-kedinasan p-6 sm:p-8 space-y-6 bg-white">
        ...
        <div className="space-y-3">
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-[#0D2A4A] font-heading leading-tight">
            {berita.judul}
          </h1>
        ...
        {/* Gambar Utama */}
        <div className="rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 max-h-[480px]">
          <img
        ...
        {/* Ringkasan Berita */}
        <div className="p-4 bg-blue-50/50 rounded-2xl border-l-4 border-[#1565C0] text-sm text-[#0D2A4A] font-medium leading-relaxed italic">
          "{berita.ringkasan}"
        </div>
        ...
        <div className="mt-8 p-4 bg-slate-50 rounded-2xl border border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-slate-600">
          ...
          <span className="font-mono text-emerald-600 font-bold bg-emerald-50 px-2 py-1 rounded border border-emerald-200">
      ...
      <section className="card-kedinasan p-6 sm:p-8 space-y-6 bg-white">
        ...
        <form onSubmit={handleKirimKomentar} className="space-y-4 bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200">
          ...
          <button
            type="submit"
            className="px-5 py-2.5 bg-[#1565C0] hover:bg-[#0D2A4A] text-white rounded-xl text-xs font-bold transition-colors flex items-center gap-2 shadow-xs"
          >
