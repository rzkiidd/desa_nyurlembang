...
export const KelolaPengaduanWarga: React.FC<KelolaPengaduanWargaProps> = ({ currentUser }) => {
  ...
  return (
    <div className="space-y-5 max-w-full overflow-x-hidden">
      {toastMsg && (
        <div className="p-3.5 sm:p-4 bg-[#0D2A4A] text-white rounded-2xl shadow-lg border border-blue-800 text-xs font-bold flex items-center gap-2 animate-in fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-200">
        <div>
          <h2 className="text-lg sm:text-xl font-bold text-[#0D2A4A] font-heading flex items-center gap-2">
            <MessageSquare className="w-5 h-5 text-[#1565C0] shrink-0" />
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="px-2.5 sm:px-3 py-1 bg-blue-50 text-[#1565C0] font-mono font-bold text-[11px] sm:text-xs rounded-xl border border-blue-200">
            Total: {list.length} Laporan
          </span>
          <span className="px-2.5 sm:px-3 py-1 bg-amber-50 text-amber-800 font-mono font-bold text-[11px] sm:text-xs rounded-xl border border-amber-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3">
        <div className="sm:col-span-2 relative">
          ...
          <input
            type="text"
            placeholder="Cari nomor tiket, nama pelapor, atau isi..."
            value={searchTerm}
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          ...
          filtered.map((item) => (
            <div
              key={item.id}
              className="p-3.5 sm:p-4 bg-white rounded-2xl border border-slate-200 hover:border-blue-300 transition-all shadow-xs space-y-3 overflow-hidden"
            >
              <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-2 border-b border-slate-100 pb-2.5">
                <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                  <span className="px-2.5 py-0.5 sm:py-1 bg-slate-100 font-mono text-[10px] sm:text-[11px] font-bold text-slate-700 rounded-lg">
                    {item.nomor_tiket}
                  </span>
              ...
              <div>
                <h4 className="font-bold text-xs sm:text-sm text-[#0D2A4A] break-words">{item.judul}</h4>
                <p className="text-xs text-slate-600 mt-1 leading-relaxed line-clamp-3 break-words">
                  {item.isi_laporan}
                </p>
              </div>

              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2.5 border-t border-slate-100 text-[11px] text-slate-500">
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                  <span className="flex items-center gap-1 font-semibold text-slate-700">
                    <User className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                    <span className="truncate max-w-[140px] sm:max-w-none">{item.rahasia ? 'Warga (Disamarkan)' : item.nama}</span>
                  </span>
      ...
      {selectedPengaduan && (
        <div className="fixed inset-0 z-[99999] bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white rounded-2xl sm:rounded-3xl max-w-xl w-full shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[92vh] flex flex-col">
            <div className="p-4 sm:p-5 border-b border-slate-200 flex items-center justify-between bg-white shrink-0">
              <div className="flex items-center gap-2 min-w-0 pr-2">
                <MessageSquare className="w-5 h-5 text-[#1565C0] shrink-0" />
                <div className="min-w-0">
                  <h3 className="font-heading font-bold text-sm sm:text-base text-[#0D2A4A] truncate">
                    Tanggapan: {selectedPengaduan.nomor_tiket}
                  </h3>
                  <div className="text-[11px] text-slate-500 truncate">{selectedPengaduan.judul}</div>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveTanggapan} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 text-xs">
              {/* Detail Pelapor */}
              <div className="p-3.5 sm:p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-2">
                ...
                <div className="pt-2 border-t border-slate-200">
                  ...
                  <p className="text-slate-800 leading-relaxed bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 break-words whitespace-pre-wrap">
                ...
                {selectedPengaduan.foto_url && (
                  <div className="pt-2 border-t border-slate-200">
                    ...
                    <img
                      ...
                      alt="Bukti Pengaduan"
                      className="max-h-48 rounded-xl object-cover border border-slate-200 max-w-full"
                    />
                  </div>
                )}
              </div>
